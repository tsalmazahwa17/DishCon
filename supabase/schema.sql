...
-- Public registration can only create donatur or penerima.
-- Admin accounts must be promoted manually by a trusted operator/service role.
...
begin
  ...
  insert into public.profiles (id, email, name, phone, role, address, organization, beneficiaries, created_at)
  values (
    new.id,
    new.email,
    ...
    coalesce((new.raw_user_meta_data ->> 'beneficiaries')::integer, 0),
    now()
  )
...
begin
  if new.role is distinct from old.role then
    ...
    if coalesce(current_setting('dishcon.allow_public_role_change', true), 'false') <> 'true' then
      raise exception 'Role publik hanya dapat ditetapkan melalui alur OAuth DishCon';
    end if;
  end if;
  return new;
end;
...
-- Dipanggil dari callback Google OAuth. Hanya dapat menetapkan role publik satu kali.
...
begin
  ...
  update public.profiles
     set role = p_role, oauth_role_set_at = now()
   where id = auth.uid()
     and role <> 'admin'
     and oauth_role_set_at is null;
end;
$$;
...
create policy "donor inserts own donations" on public.donations for insert with check (
  donor_id = auth.uid()
  and exists(select 1 from public.profiles where id = auth.uid() and role = 'donatur')
);
