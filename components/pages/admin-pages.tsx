...
import { EmptyState, MetricCard, StatusBadge, statusHint, readableStatus } from "@/components/pages/shared";
      ...
      <DialogContent className="max-h-[88vh] max-w-3xl overflow-y-auto">
        {request && <>
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            ...
            <Detail label="Status" value={readableStatus(request.status)} />
          </div>
          <div className="mt-4 rounded-3xl border border-brand-100 bg-brand-50 p-4"><b className="text-brand-900">Alur Status Manual</b><p className="mt-2 text-sm leading-6 text-brand-800">{statusHint(request.status)}</p></div><div className="mt-4 rounded-3xl border border-stone-200 p-4"><b>Catatan Penerima</b><p className="mt-2 text-sm leading-6 text-stone-600">{request.note || "Tidak ada catatan."}</p></div>
          {donation && <>
...
export function AdminRequestsPage() {
  ...
  return <>
    <section className="soft-panel p-5">
      ...
      {loading ? <p>Memuat data...</p> : filtered.length === 0 ? <EmptyState title="Belum ada pengajuan" description="Pengajuan penerima akan tampil otomatis setelah dikirim dari halaman katalog." /> : <div className="overflow-x-auto"><table className="mockup-table min-w-[1050px] w-full"><thead><tr><th>ID</th><th>Penerima</th><th>Makanan</th><th>Donatur</th><th>Porsi</th><th>Status Manual</th><th>Keterangan</th><th>Aksi</th></tr></thead><tbody>{filtered.map((item) => <tr key={item.id}><td>{item.id}</td><td>{profileLabel(profiles, item.recipient_id)}</td><td><b>{item.food_name}</b><p className="text-xs text-stone-500">{"Pengambilan mandiri"}</p></td><td>{profileLabel(profiles, item.donor_id)}</td><td>{item.portions}</td><td><StatusBadge status={item.status} /></td><td className="max-w-xs text-xs text-stone-500">{statusHint(item.status)}</td><td><div className="flex gap-2"><Button size="sm" variant="secondary" onClick={() => setSelected(item)}><FiEye /> Detail</Button><Button size="sm" onClick={() => updateRequestStatus(item.id, "approved")} disabled={item.status === "approved" || item.status === "completed"}><FiCheck /> Setujui / Belum Diambil</Button><Button size="sm" variant="secondary" onClick={() => updateRequestStatus(item.id, "rejected")} disabled={item.status === "rejected" || item.status === "completed"}><FiX /> Tolak</Button>{item.status === "approved" && <Button size="sm" onClick={() => updateRequestStatus(item.id, "completed")}>Tandai Sudah Diambil</Button>}</div></td></tr>)}</tbody></table></div>}
    </section>
