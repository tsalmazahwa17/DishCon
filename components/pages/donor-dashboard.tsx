      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
        ...
        <section className="soft-panel p-5">
          ...
          <section className="soft-panel p-5">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-xl font-black text-brand-900">Status Pengajuan Penerima</h2>
                <p className="text-sm text-stone-500">Setiap pengajuan ke donasi Anda tampil otomatis: menunggu, belum diambil, atau sudah diambil.</p>
              </div>
            </div>
            {requests.length === 0 ? (
              <EmptyState title="Belum ada pengajuan penerima" description="Jika penerima mengajukan makanan dari donasi Anda, statusnya akan muncul di sini." />
            ) : (
              <div className="overflow-x-auto">
                <table className="mockup-table min-w-[760px] w-full">
                  <thead><tr><th>ID</th><th>Makanan</th><th>Porsi</th><th>Status Manual</th><th>Keterangan</th></tr></thead>
                  <tbody>{requests.slice(0, 6).map((request) => (
                    <tr key={request.id}>
                      <td>{request.id}</td>
                      <td><b>{request.food_name}</b><p className="text-xs text-stone-500">Pengambilan mandiri</p></td>
                      <td>{request.portions} porsi</td>
                      <td><StatusBadge status={request.status} /></td>
                      <td className="max-w-xs text-xs text-stone-500">{statusHint(request.status)}</td>
                    </tr>
                  ))}</tbody>
                </table>
              </div>
