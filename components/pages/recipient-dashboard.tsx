...
import { EmptyState, MetricCard, StatusBadge, statusHint } from "@/components/pages/shared";

export function RecipientDashboardPage() {
  const { user } = useAuth();
  const { requests, donations, loading, preferences, updateRequestStatus } = useDishconData();
          ...
          <div className="soft-panel p-6">
            ...
            {requests.length === 0 ? (
              ...
              <div className="w-full max-w-full overflow-x-auto">
                <table className="mockup-table w-full min-w-[760px]"><thead><tr><th>ID</th><th>Makanan</th><th>Porsi</th><th>Status Manual</th><th>Keterangan</th><th>Tanggal</th><th>Aksi</th></tr></thead><tbody>{requests.slice(0, 5).map((r) => <tr key={r.id}><td>{r.id}</td><td>{r.food_name}</td><td>{r.portions}</td><td><StatusBadge status={r.status} /></td><td className="max-w-xs text-xs text-stone-500">{statusHint(r.status)}</td><td>{new Date(r.created_at).toLocaleDateString("id-ID")}</td><td>{r.status === "approved" ? <Button size="sm" onClick={() => updateRequestStatus(r.id, "completed")}><FiCheckCircle /> Sudah Diambil</Button> : <span className="text-xs text-stone-400">-</span>}</td></tr>)}</tbody></table>
              </div>
            )}
          </div>
        </section>
