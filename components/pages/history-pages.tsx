...
import { EmptyState, StatusBadge, statusHint } from "@/components/pages/shared";
...
export function RecipientHistoryPage() {
  const { requests, updateRequestStatus } = useDishconData();
  const [q, setQ] = useState("");
  ...
  return (
    <section className="soft-panel p-5">
      ...
      {filtered.length === 0 ? <EmptyState title="Belum ada riwayat pengajuan" description="Akun baru belum memiliki pengajuan. Setelah memilih makanan dari katalog, statusnya akan tersimpan di sini." /> : <div className="overflow-x-auto"><table className="mockup-table min-w-[900px] w-full"><thead><tr><th>ID Pengajuan</th><th>Tanggal</th><th>Nama Makanan</th><th>Porsi</th><th>Status Manual</th><th>Keterangan</th><th>Aksi</th></tr></thead><tbody>{filtered.map((r) => <tr key={r.id}><td>{r.id}</td><td>{new Date(r.created_at).toLocaleString("id-ID")}</td><td><b>{r.food_name}</b></td><td>{r.portions}</td><td><StatusBadge status={r.status} /></td><td className="max-w-xs text-xs text-stone-500">{statusHint(r.status)}</td><td>{r.status === "approved" ? <Button size="sm" onClick={() => updateRequestStatus(r.id, "completed")}><FiCheckCircle /> Tandai Sudah Diambil</Button> : <span className="text-xs text-stone-400">-</span>}</td></tr>)}</tbody></table></div>}
    </section>
  );
}
