...
function Sidebar({ role, open, onClose }: { role: UserRole; open?: boolean; onClose?: () => void }) {
  ...
  return (
    <aside className={cn("flex h-full flex-col border-r border-stone-200", admin ? "bg-brand-900 text-white" : "bg-white text-ink")}> 
      ...
      <div className="p-5">
        <div className={cn("rounded-2xl border p-4 text-sm", admin ? "border-white/10 bg-white/5 text-white/80" : "border-brand-100 bg-brand-50 text-brand-900")}>
          <p className="font-black">{admin ? "🟢 Sistem Normal" : "Pusat Bantuan"}</p>
          <p className="mt-1 leading-5 opacity-80">{admin ? "Akses admin aktif sesuai peran akun." : "Menu ini mengikuti peran akun, jadi tidak akan tertukar ke role lain."}</p>
        </div>
        <button onClick={logout} className={cn("mt-3 flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-sm font-extrabold transition", admin ? "text-white/70 hover:bg-white/10 hover:text-white" : "text-red-600 hover:bg-red-50")}>
          <FiLogOut /> Logout
        </button>
      </div>
    </aside>
