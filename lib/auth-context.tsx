  ...
  const register = useCallback(async (payload: RegisterPayload): Promise<AuthResult> => {
    ...
    if (supabase) {
      ...
      const { data, error } = await supabase.auth.signUp({
        ...
        options: {
          emailRedirectTo,
          data: { name: name.trim(), phone: phone.trim(), role, address: address || "", organization: organization || "", beneficiaries: beneficiaries || 0 }
        }
      });
    ...
    ensureDemoAdmin();
    const accounts = readLocalAccounts();
    if (accounts.some((item) => item.email.toLowerCase() === email.trim().toLowerCase())) {
