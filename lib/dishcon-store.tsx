...
function readableRequestStatus(status: FoodRequest["status"]) {
  const labels: Record<FoodRequest["status"], string> = {
...
function requestStatusMessage(foodName: string, status: FoodRequest["status"]) {
  if (status === "approved") return `${foodName} disetujui. Status saat ini: belum diambil.`;
  if (status === "completed") return `${foodName} sudah diambil. Distribusi selesai.`;
  if (status === "rejected") return `${foodName} ditolak oleh admin.`;
  if (status === "cancelled") return `${foodName} dibatalkan.`;
  return `${foodName} menunggu persetujuan admin.`;
}
...
export function useDishconData() {
  ...
  useEffect(() => {
    const sb = supabase;
    if (!sb || !user?.id || !user?.role) return;

    const channelName = `dishcon-live-${user.id}-${user.role}`;

    // Next.js development/React Strict Mode can mount effects twice.
    // Remove any existing channel with the same topic before registering
    // postgres_changes callbacks, otherwise Supabase throws:
    ...
    channel.on("postgres_changes", { event: "*", schema: "public", table: "food_requests" }, () => refresh());
    channel.on("postgres_changes", { event: "*", schema: "public", table: "donations" }, () => refresh());
    channel.on("postgres_changes", { event: "*", schema: "public", table: "notifications" }, () => refresh());
    channel.on("postgres_changes", { event: "*", schema: "public", table: "complaints" }, () => refresh());

    channel.subscribe((status) => {
      if (status === "CHANNEL_ERROR") {
        // Keep the app usable even if Realtime is not enabled in Supabase.
        console.warn("DishCon realtime channel error. Data will refresh manually.");
      }
    });
  ...
  const createRequest = useCallback(async (payload: Omit<FoodRequest, "id" | "recipient_id" | "status" | "created_at">) => {
    ...
    if (supabase) {
      ...
      await addNotification({
        ...
        message: requestStatusMessage(request.food_name, "pending"),
        type: "success",
  ...
  const updateRequestStatus = useCallback(async (id: string, status: FoodRequest["status"]) => {
    ...
    const isAdminAction = user.role === "admin";
    const isRecipientPickupAction = user.role === "penerima" && request.recipient_id === user.id && request.status === "approved" && status === "completed";
    if (!isAdminAction && !isRecipientPickupAction) return;
    ...
    if (supabase) {
      ...
      if (rpcError) {
        // Fallback khusus untuk penerima menandai makanan sudah diambil.
        // Jika RPC belum di-update, policy own request masih memungkinkan update status milik sendiri.
        if (isRecipientPickupAction) {
      ...
      const statusLabel = readableRequestStatus(status);
      await addNotification({
        user_id: request.recipient_id,
        title: `Status pengajuan: ${statusLabel}`,
        message: requestStatusMessage(request.food_name, status),
      ...
      if (request.donor_id) {
        await addNotification({
          ...
          title: `Status pengajuan penerima: ${statusLabel}`,
          message: `${request.food_name} (${request.portions} porsi) kini berstatus: ${statusLabel}.`,
          type: status === "approved" || status === "completed" ? "success" : status === "rejected" ? "warning" : "info",
          link: "/donor/dashboard"
        });
      }
      if (status === "completed" && request.donor_id) {
        const admins = data.profiles.filter((profile) => profile.role === "admin" || profile.roles?.includes("admin" as any));
        await Promise.all(admins.map((admin) => addNotification({
          user_id: admin.id,
          title: "Pengambilan selesai",
          message: `${request.food_name} sudah ditandai diambil oleh penerima.`,
          type: "success",
          link: "/admin/requests"
        })));
      }
    ...
    const notifications = [
      makeNotification(request.recipient_id, {
        title: `Status pengajuan: ${readableRequestStatus(status)}`,
        message: requestStatusMessage(request.food_name, status),
        type: status === "approved" || status === "completed" ? "success" : status === "rejected" ? "warning" : "info",
        link: "/recipient/history"
      }),
      ...(request.donor_id ? [makeNotification(request.donor_id, {
        title: `Status pengajuan penerima: ${readableRequestStatus(status)}`,
        message: `${request.food_name} (${request.portions} porsi) kini berstatus: ${readableRequestStatus(status)}.`,
        type: status === "approved" || status === "completed" ? "success" : status === "rejected" ? "warning" : "info",
        link: "/donor/dashboard"
      })] : [])
    ];
  ...
  }, [addNotification, data.donations, data.profiles, data.requests, refresh, user]);
