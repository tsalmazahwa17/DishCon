...
import { DistributionFlowVisual, ImpactPosterVisual, SafetyPosterVisual } from "@/components/landing/campaign-poster-visuals";
...
export default function LandingPage() {
  const contactEmailLabel = process.env.NEXT_PUBLIC_CONTACT_EMAIL_LABEL || "Email";
  const contactEmailHref = process.env.NEXT_PUBLIC_CONTACT_EMAIL_HREF || "https://mail.google.com/mail/?view=cm&fs=1&to=bagusganz65%40gmail.com&su=Bantuan%20DishConnect&body=Halo%20DishConnect%2C%0A%0ASaya%20ingin%20bertanya%20tentang%20";
  const contactPhoneLabel = process.env.NEXT_PUBLIC_CONTACT_PHONE_LABEL || "Telepon";
  const contactPhoneHref = process.env.NEXT_PUBLIC_CONTACT_PHONE_HREF || "https://wa.me/6287765432823";
      ...
      <section className="container-page pb-16">
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="overflow-hidden rounded-[2rem] border border-brand-100 bg-white shadow-card">
            ...
            <div className="p-6"><h3 className="font-black text-brand-900">Smart Distribution</h3><p className="mt-2 text-sm leading-7 text-stone-600">Visual formal untuk menjelaskan alur distribusi dari donatur, pengecekan AI, sampai penerima.</p></div>
          </div>
        </div>
      </section>

      <section className="container-page -mt-2 grid gap-6 pb-16 sm:grid-cols-2 lg:grid-cols-3">
        {stats.map((item) => {
